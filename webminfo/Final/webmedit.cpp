...
const long long kTrackType = 0x83; // TrackType элемент
const long long kVideo = 0xE0;
const int AUDIO_TRACK_TYPE = 2;    // Тип аудиотрека
...
// Функция для обработки элементов EBML и удаления аудиотреков
void parse_ebml_element(FILE *file, FILE *outFile, long long end_position, int depth) {
    while (ftell(file) < end_position) {
        ...
        if (element_id == kTrackEntry) {
            // Обрабатываем элемент TrackEntry
            long long track_entry_pos = ftell(file);
            bool is_audio_track = false;

            // Считываем TrackType
            while (ftell(file) < track_entry_pos + size) {
                long sub_id_length;
                long long sub_element_id = ReadID(file, ftell(file), &sub_id_length);
                if (sub_element_id == kTrackType) {
                    long sub_size_length;
                    ReadSize(file, ftell(file), &sub_size_length);
                    uint8_t track_type;
                    fread(&track_type, 1, 1, file);
                    if (track_type == AUDIO_TRACK_TYPE) {
                        is_audio_track = true;  // Это аудиотрек
                    }
                    break;
                } else {
                    long sub_size_length;
                    long long sub_size = ReadSize(file, ftell(file), &sub_size_length);
                    fseek(file, sub_size, SEEK_CUR);  // Пропускаем другие элементы
                }
            }

            if (!is_audio_track) {
                // Копируем TrackEntry, если это не аудиотрек
                char *buffer = (char *)malloc(size + id_length + size_length);
                fread(buffer, size + id_length + size_length - 1, 1, file);
                fwrite(buffer, size + id_length + size_length - 1, 1, outFile);
                free(buffer);
            } else {
                // Пропускаем аудиотрек
                fseek(file, element_end, SEEK_SET);
                printf("\nAudio track removed.\n\n");
            }
        } else if (element_id == kEBML 
            || element_id == kSegment 
            || element_id == kTracks 
            || element_id == kVideo) {
            // Если это контейнер, рекурсивно обработаем его содержимое
            parse_ebml_element(file, outFile, element_end, depth + 1);
        } else {
            // Пропуск элемента, перемещаем указатель чтения и копируем содержимое
            char *buffer = (char *)malloc(size + id_length + size_length);
            fread(buffer, size + id_length + size_length, 1, file);
            fseek(file, element_end, SEEK_SET);
            fwrite(buffer, size + id_length + size_length, 1, outFile);
            free(buffer);
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    FILE *inFile = fopen(input_file, "r+b");
    if (!inFile) {
        printf("Could not open the input file!\n");
        return 1;
    }

    FILE *outFile = fopen(output_file, "w+b");
    if (!outFile) {
    ...
    // Парсим элементы и удаляем аудиотреки
    parse_ebml_element(inFile, outFile, file_size, 0);

    fclose(inFile);
    fclose(outFile);

    printf("Processing complete. Audio tracks removed from the output file.\n");
